...
Setup
Review the Assignments page for general questions about assignments.
As in assignment 1, make sure you have Python 3, NumPy and GoGui 1.51 set up. You can review the procedures under Lecture 3 activities.
Also, check the Assignment 2 preview and "more preview" in the lecture slides.
Download assignment2.tgz and expand it. The directory assignment2 contains:
...
For reference, download the Lecture 8 python codes which include solvers for TicTacToe.
...
Implement a new GTP command solve which attempts to compute the winner of the current position, assuming perfect play by both, within the current time limit.
...
If the game is not over yet, the program should use the solver to try to play perfectly. The solver must respect the current time limit as described under timelimit above. Note that the time limit is per move, not for the whole game.
If the game is not solved within the timelimit, or if toPlay is losing, then the genmove command should just play a random move.
Public Test Cases
...
Your file must be a valid tgz file named assignment2.tgz which can be uncompressed with tar -xzvf assignment2.tgz
